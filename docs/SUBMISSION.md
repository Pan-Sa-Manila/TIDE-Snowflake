...
| Prototype deployed link — judges access it directly | Nico → Keith | **deployed 4 Aug**: `TIDE.TRIAGE.TIDE_APP` — `https://app.snowflake.com/OXAZYMD/GQ85743/#/streamlit-apps/TIDE.TRIAGE.TIDE_APP` — granted to all three personas and `TIDE_JUDGE`. **Verify this link resolves before submitting**: the `region/account` form that `snow streamlit get-url` prints (`.../ap-southeast-7.aws/gq85743/...`) returns *Page not found* in a browser. Snowsight wants `organisation/account`. |
